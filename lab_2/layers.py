...
class FC(Layer):
  ...
  def backward_params(self, grads):
    ...
    grad_bias = np.sum(grads,axis = 0) #Sumiraj grads po svim podatcima
    return [[self.weights, grad_weights], [self.bias, grad_bias], self.name]
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    linear = np.sum(x*y,axis = 1) #Produkt x i y član po član i sua po klasama

    return np.mean(logsum - linear)
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    return 0.5*self.weight_decay*np.sum(np.linalg.norm(self.weights)**2 )
