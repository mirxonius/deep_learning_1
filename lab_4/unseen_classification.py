...
if __name__ == "__main__":
    ...
    ds_removed_train = MNISTMetricDataset(mnist_download_root,split='train',
    remove_class = 0) #[0,1,2,3,4,5,6,7])
    ...
    train_removed_loader = DataLoader(
        ds_removed_train,
        batch_size=64,
        shuffle=True,
        pin_memory=True,
    ...
    for epoch in range(epochs):
        print(f"Epoch: {epoch}")
        t0 = time.time_ns()
        train_loss = train(model, optimizer, train_removed_loader, device)
    ...
    torch.save(model.state_dict(),"has_unseen.pt")