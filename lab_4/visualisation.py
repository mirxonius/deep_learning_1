...
if __name__ == '__main__':
    ...
    plt.show()
    plt.figure()
    print("Fitting PCA from feature representation - missing classes")
    with torch.no_grad():
        model.eval()
        test_rep = model.get_features(X.unsqueeze(1))
        test_rep2d = torch.pca_lowrank(test_rep, 2)[0].cpu()
        plt.scatter(test_rep2d[:, 0], test_rep2d[:, 1], color=colormap[Y[:]] / 255., s=5)
        plt.show()


    model.load_state_dict(torch.load("seen_all.pt"))
    model.eval()
    print("Fitting PCA from feature representation - all classes")
    with torch.no_grad():
        model.eval()
